...
//	final float MAXCOLOR = 0.8f;
    ...
    public void readPoints(File file, int start, int end) {
        ...
        try {
            ...
            while (tok.ttype != TT_EOF) {
                //System.out.println(" all "  + tok.ttype + " " + tok.nval);
    ...
    //            if (tok.nextToken() != TT_NUMBER)
     ...
     //           altTempInput = (float)tok.nval;        
                ...
                if (tok.nextToken() != TT_WORD) {
                    stringInput = ".";
                 //   System.out.println(" pai "  + tok.ttype + " " + tok.nval + " " +
                  //         tok.lineno() );
                //    throw new Exception ("Residue missing at input line " + tok.lineno());
                }
                else {               
               // System.out.println(" pai "  + tok.ttype + " " + tok.sval);
                stringInput = tok.sval;
                }
                if (tok.nextToken() != TT_WORD ) {
                   // System.out.println(" pai "  + "ttype " + tok.ttype + " " + tok.nval);
                   pairedStringInput = ".";
                  //  throw new Exception ("Residue1 missing at input line " + tok.lineno());
                }
                else
                  pairedStringInput = tok.sval;
    ...
    //		System.out.println("cov= " + tempInput + "res = " + stringInput + " nmb = " + tempExtra);
            ...
            //        altCoverage.add(altTempInput);
                    ...
                    pairedRes.add(pairedStringInput);
    ...
    //		 	 		g.setPaint(myColor( coverage.get(i)));
                ...
                //        boundary.moveTo(margin + delta, marginTop + position + proteinWidth);
                 //       boundary.lineTo(margin + delta + lineWidth, marginTop + position + proteinWidth);
                        ...
                        String pairedAxes = "";
                        axes = residue.get(i);
                        pairedAxes = pairedRes.get(i);
                        
        //	if (i < x.length - 5)
                        //	g.drawString(axes,margin + delta + 1.5f, marginTop + position + stripe + 25) ;
                        g.drawString(axes,margin + delta + 1.5f, marginTop + position + proteinWidth / 2 +
                                                            (int)(FONTSIZE*.4)) ;
...
//                                        proteinWidth / 2 + 2*(int)(FONTSIZE*.4));
//
//                        }
                         g.drawString(pairedAxes,margin + delta + 1.5f, marginTop + position + proteinWidth / 2 +
                                                            (int)(FONTSIZE*.4) * 2) ;
                        if(countResidues%10 == 0){// || (countResidues+1)%10 == 0) {
//			      		if (i%50== 0 || (i+1)%10 == 0) {
                            axes = Integer.toString(resNumbers.get(i));//Integer.toString(shift + i+1);
                 //           g.drawString(axes,margin + delta, marginTop + position +
                   //                     proteinWidth + 2*(int)(FONTSIZE*.4));
                            g.drawString(axes,margin + delta, marginTop + position +
                                        proteinWidth / 2 + 3*(int)(FONTSIZE*.4));
    ...
    public static void main(String[] args) throws Exception {
        if (args.length < 2) {
            System.out.println("Usage: java TwoSeq <input file> \"figure name\" [scaling factor(default 1)]");
            System.exit(0);
        }
...
// 		    end = 10000;
        if (args.length == 3) {
                plot = new TwoSeq(Float.parseFloat(args[2]));
        }
        else
                plot = new TwoSeq();
