...
/***********************************************************************/
int chisquare ( int * pool_population, int * node_population, int no_bins, int kstr, double *df, double *chsq, double *prob) {

    /* normalize distributions */

    int gammq(double a, double x, double * retval);
    int j;
    double pool_total, node_total;
    double temp;
    ...
    node_total = 0;
    for (j=0; j < no_bins;j++) {
	pool_total +=  pool_population[j];
	node_total +=  node_population[j];
    }

    p_prefactor = sqrt (node_total/pool_total);
    n_prefactor = 1.0/p_prefactor;
  
    *df = no_bins - kstr;
    *chsq = 0.0;
    for (j=0; j < no_bins;j++) {
	if (pool_population[j] == 0.0 && node_population[j] == 0.0) {
	    --(*df);   
	} else {  
	    temp   = p_prefactor* pool_population[j] - n_prefactor*node_population[j];
	    *chsq += temp*temp/(pool_population[j]+node_population[j]);
	}
    }
