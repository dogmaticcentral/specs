...
int int_trace (Alignment * alignment, Tree * tree, int * similar_to, double *score){
    ...
    int int_trace_recursive_part ( Node *node,  int * similar_to, int pos );
  
    /* calculate the score */
    for ( pos=0; pos < alignment->length; pos++) {
 	int_trace_recursive_part  ( tree->root, similar_to, pos);
...
int int_trace_recursive_part ( Node *node,  int * similar_to, int pos ) {
	...
	int_trace_recursive_part (node->left, similar_to, pos);
	int_trace_recursive_part (node->right, similar_to, pos);
